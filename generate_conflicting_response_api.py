...
system_response_template = (
    "You are a conflicting response generator for forums. You will be given a query that you will analyze. Your task is to provide a conflicting statement like human do that should be simple, concise, and human-like. Keep the response to a maximum of 50 words. Use everyday words that a child could understand. Avoid slang, jargon, or complex words."
)
...
response_prompt_template = ChatPromptTemplate.from_messages(
    [
        ...
        ("user", "Please generate a conflicting response using the text which is '{text}' in a '{tone}' tone of voice.")
    ]
)
...
@app.post("/generate-conflicting-response", response_model=TextResponse, dependencies=[Depends(verify_token)])
